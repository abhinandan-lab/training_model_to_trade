...
# Create info label - using a simple approach
...
def update_chart():
    ...
    for i, (x, open_val, close_val, low_val, high_val) in enumerate(candlestick_data):
        ...
        if body_height > 0:
            ...
            brush = pg.mkBrush(color) if close_val < open_val else pg.mkBrush(None)  # Fill only for red candles
            
            # Create rectangle item
            rect = pg.QtWidgets.QGraphicsRectItem(x-0.3, body_y, 0.6, body_height)
            rect.setPen(pg.mkPen(color, width=1))
            if close_val < open_val:  # Red candle - filled
                rect.setBrush(pg.mkBrush(color))
            else:  # Green candle - hollow
